...
튜토리얼을 마친 재방문자는 홈의 큰 버튼에서 45초 퀵 시프트를 바로 시작합니다. 퀵 시프트는 비 오는 날, 주말 러시, 전력 불안, 이불 세탁의 날처럼 매장 조건·주요 손님·오염·첫 사건·추가 목표가 함께 설계된 6개 구성을 날짜와 플레이 기록에 따라 순환합니다. 75초 기본 영업이나 다른 시간·난이도를 고르려면 바로 아래 `시간·난이도와 목표 선택`을 사용합니다.
...
- 손님·오염·사건·업그레이드 도감
- 간판 10종·바닥 6종·벽지 6종·화분 6종, 총 28종 매장 꾸미기
...
- 홈: 100dvh 전체 페이지에서 `MANAGER DESK`·45초 바로 영업·계획 선택·보조 버튼을 354px 콘텐츠 폭으로 넓히고, 퀵 시작과 계획 선택은 하나의 54px 높이 행동 막대로 결합
...
| `laundry-game-release.cjs` | 퀵 시프트·LAST SAVE·조작 지표를 포함한 필수 UI·기능·PWA 자산·PNG 크기와 온라인 기능 제외 상태 |
