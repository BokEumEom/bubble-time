...
| 모드 | 시간 | 특징 |
| --- | ---: | --- |
| 빠른 영업 | 45초 | 홈에서 한 번 눌러 시작하는 순환형 미니 시프트 |
...
튜토리얼을 마친 재방문자의 홈에는 같은 폭의 독립된 2열 카드가 표시됩니다. `45초 바로 영업`은 오늘의 퀵 시프트를 준비 화면 없이 시작하고, `계획 선택`은 45·75·120초·무한 영업과 난이도·목표를 직접 고르는 화면으로 이동합니다. 두 행동은 겹치지 않는 별도 클릭 영역이라 모바일에서도 목적을 바로 구분할 수 있습니다.

퀵 시프트는 비 오는 날, 주말 러시, 전력 불안, 이불 세탁의 날처럼 매장 조건·주요 손님·오염·사건·추가 목표가 함께 설계된 6개 구성을 날짜와 플레이 기록에 따라 순환합니다. 각 구성은 설명만 다른 것이 아니라 실제 보너스와 특별 목표가 다릅니다.
...
| 단골 집중 시간 | 단골 인내심 +35% | 단골 3명 응대 |

퀵 시프트가 끝나면 `완주`, `오조작 2회 이하`, `시나리오 특별 목표`를 각각 별 하나로 평가합니다. 시나리오별 최고 점수·등급·최고 별·직전 점수와 오조작 수가 저장되며, 결과에서 직전 판 대비 변화와 누적 별을 확인할 수 있습니다. `다음 퀵`을 누르면 홈이나 계획 화면을 거치지 않고 다음 시나리오가 바로 시작됩니다.
...
- 튜토리얼: 목표 HUD가 숨겨져도 639px 기계 보드와 110px 도구판 행을 고정. 정전·세제 단계에서는 안내 카드를 중앙으로 옮겨 상단 설비와 하단 도구를 모두 누를 수 있게 하고 현재 필요한 도구를 함께 강조
...
| 저장 키 | 내용 |
| --- | --- |
| `bubbleTime75.progression.v1` | 수익, 강화, 도감, 설정, 업적, 영업 기록, 모드별 최고 기록, 퀵 시프트별 별·최고·직전 기록 |
...
- 선택한 모드·난이도·추가 목표
- 점수, 환불, 콤보와 영업 통계
- 퀵 시프트 구성, 첫 유효 행동, 오조작, LAST SAVE 기록
- 대기 손님과 인내심
- 기계의 오염·고장·작업 진행도
