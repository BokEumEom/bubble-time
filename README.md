...
- 게임 HUD: 38px 상단 행에는 남은 시간·매출·환불과 일시정지만 두고, 콤보·만족도·대기 인원은 바로 아래 상태 행으로 분리. HUD 카드와 목표 사이의 가로 간격을 1~2px로 정리하고 최고 기록과 중복 설정·음향 버튼은 영업 중 숨김
- 매장: 390×844에서 실제 플레이 영역의 639px 높이와 비율을 유지하고, 세탁기 6대와 건조기 6대를 각각 3열 2행으로 배치해 12대 모두 첫 화면에 표시. 세제 탱크와 차단기는 매장 헤더 오른쪽, 사건·손님·첫 영업 안내는 왼쪽 30px 칩으로 분리
- 도구판: 여섯 도구를 42px 높이의 3열 2행으로 배치하고 도구명을 11px 이상으로 유지하며 버튼 사이는 2px로 정리. 하단 설비 바를 헤더로 옮겨 마지막 기계와 도구 버튼 사이를 36px에서 18px로 축소하고 보드 테두리에 도구판을 바로 연결
- 결과: 주요 정보 간격을 8px로 통일해 핵심 점수와 등급을 먼저 표시하고, 목표·손님 유형·운영 분석은 `상세 영업 기록`으로 접어 제공
- 관리·설정: 하단 네 그룹 탐색을 유지하면서 설명, 버튼, 설정 라벨 글자 크기를 확대

모바일 홈·영업 준비의 핵심 행동과 게임 HUD·기계 12대·도구 6개가 390×844 첫 화면 안에 들어오는지 회귀 검사합니다. 354px 홈 버튼, 설정 탭 양방향 슬라이드, 깜박임 없는 복귀, 639px 매장 비율, 18px 이내의 기계–도구 간격과 서로 겹치지 않는 헤더 설비·안내도 함께 검사합니다.
...
| `laundry-game-ui.cjs` | 실제 브라우저에서 전용 홈·관리 센터 전환, 모드 선택, 오염 청소, 고장·정전·세제 대응, 체크포인트 복구, 결과→홈 이동, 모바일 354px 홈 버튼·설정 탭 양방향 슬라이드·깜박임 없는 복귀·결과 간격·639px 매장 비율·18px 기계–도구 간격·분리된 헤더 설비와 안내·3×2 도구판·고정 탐색 |
